...
public partial class PostedJobsPageViewModel : BaseViewModel
{
    ...
    private async Task GetJobsAsync()
    {
        try
        {
            ...
            var jobs = await _db.GetAllAsync(JobEntity.Node, selector);
            ...
            foreach (var job in jobs)
            {
                ...
                if (dateComparisonResult >= 0)
                {
                    ...
                    var sector = await _db.GetByIdAsync(SectorEntity.Node, sectorPredicate, sectorSelector);
                    ...
                    var company = await _db.GetByIdAsync(UserEntity.Node, userPredicate, userSelector);
